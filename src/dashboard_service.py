...
def _resolve_hf_token() -> str | None:
    for key in HF_AUTH_TOKEN_ENV_KEYS:
        value = os.getenv(key, "").strip()
        if value:
            return value

    secret_value = _get_streamlit_secret(*HF_AUTH_TOKEN_ENV_KEYS)
    if secret_value:
        return secret_value
...
HF_MODEL_TOKEN = _resolve_hf_token()


def _copy_download_to_target(downloaded_path: Path, target_path: Path) -> Path:
...
def _list_remote_repo_files(repo_id: str, revision: str | None) -> set[str]:
    ...
    with _hf_repo_files_lock:
        _hf_repo_files_cache[cache_key] = repo_files
    return repo_files


def _resolve_remote_model_filename(target_path: Path) -> str:
    candidate_names = HF_MODEL_FILE_ALIASES.get(target_path.name, (target_path.name,))
    repo_files = _list_remote_repo_files(HF_MODEL_REPO_ID, HF_MODEL_REVISION)
    ...
    raise ServiceConfigurationError(
        f"None of the expected files for `{target_path.name}` were found in "
        f"Hugging Face repo `{HF_MODEL_REPO_ID}`. Checked: {', '.join(candidate_names)}."
    )
...
def _download_from_hugging_face(target_path: Path) -> Path:
    ...
    try:
        remote_filename = _resolve_remote_model_filename(target_path)
        _emit_runtime_log(
...
"""
    ...
    local_path = _copy_download_to_target(Path(downloaded_path), target_path)
...
def get_runtime_diagnostics() -> list[str]:
    ...
    if (
        ...
        if HF_MODEL_TOKEN:
            issues.append("Hugging Face authentication token detected for model downloads.")
        else:
            issues.append(
    ...
    if missing_tokenizer_files and ENABLE_URL_MODEL and not HF_AUTO_DOWNLOAD:
        issues.append(
            "DistilBERT tokenizer files are missing locally. URL predictions cannot run "
            "until the tokenizer is added."
        )
    ...
    if not ENABLE_URL_MODEL:
        issues.append("URL model is disabled by the current runtime profile, so URL predictions cannot run.")
    if not ENABLE_IMAGE_MODEL:
...
def is_url_model_ready() -> bool:
    return _url_bundle_cache is not None
